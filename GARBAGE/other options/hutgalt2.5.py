"""
Trash Detection System with Tkinter GUI - OPTIMIZED FOR 30 FPS
Color-coded: GREEN = Recyclable, RED = Non-Recyclable
WITH HUMAN DETECTION FILTER

Performance Optimizations for 30 FPS:
- Frame rate control with precise timing
- Optimized detection size (480p)
- Reduced human detection frequency
...
class TrashDetectorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Trash Detection System - 30 FPS Optimized")
        ...
        # Performance settings - OPTIMIZED FOR 30 FPS
        self.target_fps = 30
        self.frame_delay = 1.0 / 30  # Target frame time
        self.process_every_n_frames = 1  # Process every frame
        self.human_detect_every_n_frames = 10  # Detect humans every 10 frames
        self.resize_for_detection = True
        self.detection_size = 480  # Smaller = faster
    ...
    def setup_left_panel(self, parent):
        ...
        ttk.Label(title_frame, text="⚡ 30 FPS", font=("Arial", 10, "bold"), foreground="green").pack()
        ...
        self.perf_var = tk.StringVar(value="fast")
        perf_frame = ttk.Frame(settings_frame)
        perf_frame.pack(fill=tk.X, pady=5)
        
        ttk.Radiobutton(perf_frame, text="30 FPS (Optimized)", variable=self.perf_var, 
                       value="fast", command=self.change_performance_mode).pack(anchor=tk.W)
        ttk.Radiobutton(perf_frame, text="Balanced (25 FPS)", variable=self.perf_var, 
                       value="balanced", command=self.change_performance_mode).pack(anchor=tk.W)
        ttk.Radiobutton(perf_frame, text="Quality (15-20 FPS)", variable=self.perf_var, 
    ...
    def change_performance_mode(self):
        ...
        if mode == "fast":
            ...
            self.human_detect_every_n_frames = 10
            self.resize_for_detection = True
            self.detection_size = 480
            print("Performance: 30 FPS OPTIMIZED mode")
            
        elif mode == "balanced":
            self.target_fps = 25
            self.frame_delay = 1.0 / 25
            self.process_every_n_frames = 1
            self.human_detect_every_n_frames = 7
            self.resize_for_detection = True
            self.detection_size = 640
            print("Performance: BALANCED mode (25 FPS)")
            
        else:  # quality
            self.target_fps = 20
            self.frame_delay = 1.0 / 20
            self.process_every_n_frames = 1
            self.human_detect_every_n_frames = 5
            self.resize_for_detection = False
            self.detection_size = 1280
            print("Performance: QUALITY mode (15-20 FPS)")
    ...
    def start_detection(self):
        ...
        self.update_status("Detection started! Target: 30 FPS")
    ...
    def detection_loop(self):
        ...
        print("\n🚀 Starting 30 FPS detection loop...")
        print(f"Detection size: {self.detection_size}px")
        print(f"Human detection: every {self.human_detect_every_n_frames} frames")
        ...
        while self.is_running:
            try:
                ...
                if frame_count % 30 == 0:
                    ...
                    print(f"Frame {frame_count}: FPS={int(avg_fps)}")
                
                # Frame rate control for 30 FPS
    ...
    def toggle_pause(self):
        ...
        if self.is_paused:
            ...
            self.update_status("Running... Target: 30 FPS")
