"""
Trash Detection System with Tkinter GUI - OPTIMIZED FOR HIGH FPS (MAX 60 FPS)
...
class TrashDetectorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Trash Detection System - AI Recycling Classifier (OPTIMIZED - 60 FPS)")
        ...
        self.target_fps = 60  # Maximum FPS limit
        self.frame_time = 1.0 / self.target_fps  # Time per frame in seconds (0.0167s for 60 FPS)
    ...
    def setup_left_panel(self, parent):
        ...
        ttk.Label(title_frame, text="⚡ OPTIMIZED (60 FPS)", font=("Arial", 9), foreground="green").pack()
    ...
    def open_camera(self):
        ...
        try:
            ...
            # Configure camera - set to 60 FPS max
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
            self.cap.set(cv2.CAP_PROP_FPS, 60)  # Set camera to 60 FPS
    ...
    def detection_loop(self):
        """Main detection loop - OPTIMIZED with 60 FPS limit"""
        prev_time = time.time()
        frame_count = 0
        human_boxes = []
        
        print("\n🚀 Starting OPTIMIZED detection loop (60 FPS max)...")
    ...
    def draw_frame_ui(self, img, fps, human_boxes):
        ...
        # Color code FPS based on target (green if near 60, yellow if moderate, red if low)
        if fps >= 50:
        ...
        cv2.putText(img, f"FPS: {int(fps)}/60", (20, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, fps_color, 2)
