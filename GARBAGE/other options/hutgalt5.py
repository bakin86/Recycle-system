"""
Trash Detection System with Tkinter GUI - ADJUSTABLE FPS
...
- Adjustable FPS limit (10-120 FPS)
"""
...
class TrashDetectorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Trash Detection System - AI Recycling Classifier (ADJUSTABLE FPS)")
        ...
        self.process_every_n_frames = 1  # Process every frame (1), or skip frames (2, 3, etc.)
        self.human_detect_every_n_frames = 5  # Detect humans less frequently
        self.resize_for_detection = True  # Resize frames before detection
        self.detection_size = 640  # Size for detection (smaller = faster)
        self.target_fps = 30  # Maximum FPS limit (adjustable)
        self.frame_time = 1.0 / self.target_fps  # Time per frame in seconds
    ...
    def setup_left_panel(self, parent):
        ...
        ttk.Label(title_frame, text="⚡ ADJUSTABLE FPS", font=("Arial", 9), foreground="green").pack()
        ...
        # FPS Target slider
        ttk.Label(settings_frame, text="Target FPS Limit:").pack(anchor=tk.W, pady=(10, 0))
        
        fps_container = ttk.Frame(settings_frame)
        fps_container.pack(fill=tk.X, pady=(5, 10))
        ...
        self.fps_limit_label = ttk.Label(fps_container, text="30", width=5)
        self.fps_limit_label.pack(side=tk.LEFT, padx=(5, 0))
        ...
        ttk.Radiobutton(perf_frame, text="Max FPS (skip frames)", variable=self.perf_var, 
                       value="fast", command=self.change_performance_mode).pack(anchor=tk.W)
        ttk.Radiobutton(perf_frame, text="Balanced", variable=self.perf_var, 
                       value="balanced", command=self.change_performance_mode).pack(anchor=tk.W)
        ttk.Radiobutton(perf_frame, text="Max Quality (slower)", variable=self.perf_var, 
    ...
    def change_performance_mode(self):
        ...
        if mode == "fast":
            self.process_every_n_frames = 2  # Skip every other frame
            self.human_detect_every_n_frames = 6
            self.resize_for_detection = True
            self.detection_size = 480
            print("Performance: MAX FPS mode (frame skipping enabled)")
            ...
            print("Performance: MAX QUALITY mode (slower)")
    ...
    def open_camera(self):
        ...
        try:
            ...
            # Try default if DirectShow failed
            ...
            # Configure camera - set to target FPS
            self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
            self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
            self.cap.set(cv2.CAP_PROP_FPS, self.target_fps)  # Set camera to target FPS
    ...
    def load_video(self):
        ...
        try:
            ...
            self.update_status(f"Video loaded: {Path(file_path).name}\nDuration: {duration:.1f}s, FPS: {fps:.1f} (limited to {self.target_fps})")
            self.start_btn.config(state=tk.NORMAL)
    ...
    def detection_loop(self):
        """Main detection loop - OPTIMIZED with adjustable FPS limit"""
        prev_time = time.time()
        frame_count = 0
        human_boxes = []
        
        print("\n🚀 Starting OPTIMIZED detection loop...")
        print(f"Target FPS: {self.target_fps}")
        print(f"Frame time: {self.frame_time:.4f}s")
        print(f"Frame skip: {self.process_every_n_frames}")
        print(f"Detection size: {self.detection_size}")
        
        while self.is_running:
            try:
                loop_start_time = time.time()  # Track loop start for FPS limiting
                ...
                if should_process:
                    # OPTIMIZATION: Resize for detection if enabled
                    ...
                    # OPTIMIZATION: Detect humans less frequently
                ...
                if frame_count % 30 == 0:
                    print(f"Frame {frame_count}: FPS={int(fps)}")
                
                # FPS LIMITING: Calculate remaining time and sleep
                loop_elapsed = time.time() - loop_start_time
                sleep_time = self.frame_time - loop_elapsed
    ...
    def draw_frame_ui(self, img, fps, human_boxes):
        ...
        # Color code FPS based on target (green if near target, yellow if moderate, red if low)
        fps_good_threshold = self.target_fps * 0.83  # 83% of target
        fps_acceptable_threshold = self.target_fps * 0.50  # 50% of target
        
        if fps >= fps_good_threshold:
            fps_color = (0, 255, 0)  # Green - good
        elif fps >= fps_acceptable_threshold:
            fps_color = (0, 255, 255)  # Yellow - acceptable
        else:
            fps_color = (0, 0, 255)  # Red - low
        
        cv2.putText(img, f"FPS: {int(fps)}/{self.target_fps}", (20, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, fps_color, 2)
    ...
    def display_frame(self, frame):
        """Display frame on canvas - OPTIMIZED"""
        ...
        if canvas_width > 1 and canvas_height > 1:
            ...
            # OPTIMIZATION: Use INTER_LINEAR for faster resizing
    ...
    def on_fps_change(self, value):
        ...
        self.fps_limit_label.config(text=str(new_fps))
        print(f"FPS limit changed to: {new_fps} (frame time: {self.frame_time:.4f}s)")
